...
(function (dt) {
    var RemoteState = (function () {
        ...
        RemoteState.prototype.generateStateData = function (oSettings) {
            var i, iLen;
            var api = this.dt.api;
        ...
        RemoteState.prototype.loadState = function (data, dtInitialized) {
            var oSettings = this.dt.settings;
            var i, ien;
            ...
            for (i = 0, ien = savedSort.length; i < ien; i++) {
                oSettings.aaSorting.push(savedSort[i][0] >= columns.length ? [0, savedSort[i][1]] : savedSort[i]);
            }
            ...
            if ($.isArray(data.colReorder) && data.colReorder.length == columns.length) {
                if (dtInitialized) {
                    ...
                    oSettings.oInit.oColReorder.aiOrder = oSettings.oInit.colReorder.order = data.colReorder;
                }
            }
