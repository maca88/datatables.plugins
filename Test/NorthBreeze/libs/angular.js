...
(function(window, document, undefined) {
  ...
  function minErr(module) {
    return function() {
      ...
      message = message + '\nhttp://errors.angularjs.org/1.3.0-beta.16/' + (module ? module + '/' : '') + code;
...
*/
  ...
  var manualLowercase = function(s) { /* jshint bitwise: false */
    return isString(s) ? s.replace(/[A-Z]/g, function(ch) {
      return String.fromCharCode(ch.charCodeAt(0) | 32);
    }) : s;
  };
  var manualUppercase = function(s) { /* jshint bitwise: false */
    return isString(s) ? s.replace(/[a-z]/g, function(ch) {
      return String.fromCharCode(ch.charCodeAt(0) & ~32);
    }) : s;
  };
  ...
  if (msie < 9) {
    nodeName_ = function(element) {
      ...
      return lowercase((element.scopeName && element.scopeName != 'HTML') ? element.scopeName + ':' + element.nodeName : element.nodeName);
    };
  } else {
    nodeName_ = function(element) {
  ...
  function setupModuleLoader(window) {
    ...
    return ensure(angular, 'module', function() { /** @type {Object.<string, angular.Module>} */
      var modules = {};
 ...
 */
  ...
  function jqLiteBuildFragment(html, context) {
    var elem, tmp, tag, wrap, fragment = context.createDocumentFragment(),
        nodes = [],
        i;
  ...
  function jqLiteDealoc(element, onlyDescendants) {
    ...
    if (element.childNodes && element.childNodes.length) {
      ...
      var descendants = element.getElementsByTagName ? element.getElementsByTagName('*') : element.querySelectorAll ? element.querySelectorAll('*') : [];
          ...
          compile: function() {
            return {
              pre: function attrInterpolatePreLinkFn(scope, element, attr) {
                ...
                interpolateFn = $interpolate(attr[name], true, getTrustedContext(node, name), ALL_OR_NOTHING_ATTRS[name]);
        ...
        if ($rootElement) {
          for (i = 0, ii = $rootElement.length; i < ii; i++) {
            if ($rootElement[i] == firstElementToRemove) {
              ...
              for (var j = i, j2 = j + removeCount - 1, jj = $rootElement.length;
              j < jj; j++, j2++) {
                if (j2 < jj) {
...
/*
          ...
          if (href.indexOf('://') < 0) { // Ignore absolute URLs
            ...
            if (href[0] == '/') {
              ...
              for (var i = 0; i < parts.length; i++) {
                if (parts[i] == ".") continue;
                else if (parts[i] == "..") stack.pop();
                else if (parts[i].length) stack.push(parts[i]);
              }
  ...
  var OPERATORS = { /* jshint bitwise : false */
    'null': function() {
      return null;
    },
    'true': function() {
  ...
  function $ParseProvider() {
    ...
    this.$get = ['$filter', '$sniffer', function($filter, $sniffer) {
      ...
      return function(exp, interceptorFn) {
        var parsedExpression, oneTime, cacheKey = (exp = trim(exp));
...
*/
  ...
  function parseConstantExpr($parse, context, name, expression, fallback) {
    ...
    if (isDefined(expression)) {
      ...
      if (!parseFn.constant) {
        throw new minErr('ngModel')('constexpr', 'Expected constant expression for `{0}`, but saw ' + '`{1}`.', name, expression);
      }
   ...
   *
   */
  var NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse', '$animate', '$timeout', function($scope, $exceptionHandler, $attr, $element, $parse, $animate, $timeout) {
    ...
    this.$commitViewValue = function(revalidate) {
      ...
      if (ctrl.$modelValue !== modelValue && (isUndefined(ctrl.$$invalidModelValue) || ctrl.$$invalidModelValue != modelValue)) {
    ...
    this.$$writeModelToScope = function() {
      ...
      if (ctrl.$options && ctrl.$options.getterSetter && isFunction(getterSetter = ngModelGet($scope))) {
      ...
      link: function($scope, $element, $attr, ctrl, $transclude) {
        ...
        $scope.$watchCollection(rhs, function ngRepeatAction(collection) {
          var index, length, previousNode = $element[0],
              ...
              nextBlockMap = {},
              arrayLength, key, value, // key/value of iteration
